...
> DealResponse create_deal_renew(id, renew_create_params)
...
id = 56 # Integer | 取引ID
renew_create_params = UnofficialFreeeSdk::RenewCreateParams.new # RenewCreateParams | 取引（収入／支出）に対する+更新の情報

begin
  #取引（収入／支出）に対する+更新の作成
  result = api_instance.create_deal_renew(id, renew_create_params)
  p result
rescue UnofficialFreeeSdk::ApiError => e
  puts "Exception when calling RenewsApi->create_deal_renew: #{e}"
end
```
...
> DealResponse update_deal_renew(id, renew_id, renew_update_params)
...
id = 56 # Integer | 取引ID
renew_id = 56 # Integer | +更新ID
renew_update_params = UnofficialFreeeSdk::RenewUpdateParams.new # RenewUpdateParams | +更新の更新情報

begin
  #取引（収入／支出）の+更新の更新
  result = api_instance.update_deal_renew(id, renew_id, renew_update_params)
  p result
rescue UnofficialFreeeSdk::ApiError => e
  puts "Exception when calling RenewsApi->update_deal_renew: #{e}"
end
```
