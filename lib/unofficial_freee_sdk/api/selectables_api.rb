...
module UnofficialFreeeSdk
  class SelectablesApi
    ...
    # @option opts [String] :includes 取得する項目(項目: account_item)
    # @return [SelectablesIndexResponse]
    def get_forms_selectables(company_id, opts = {})
      data, _status_code, _headers = get_forms_selectables_with_http_info(company_id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :includes 取得する項目(項目: account_item)
    # @return [Array<(SelectablesIndexResponse, Integer, Hash)>] SelectablesIndexResponse data, response status code and response headers
    def get_forms_selectables_with_http_info(company_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SelectablesApi.get_forms_selectables ...'
      end
      # verify the required parameter 'company_id' is set
      if @api_client.config.client_side_validation && company_id.nil?
        fail ArgumentError, "Missing the required parameter 'company_id' when calling SelectablesApi.get_forms_selectables"
      end
      if @api_client.config.client_side_validation && company_id > 4294967295
        fail ArgumentError, 'invalid value for "company_id" when calling SelectablesApi.get_forms_selectables, must be smaller than or equal to 4294967295.'
      end

      if @api_client.config.client_side_validation && company_id < 1
        fail ArgumentError, 'invalid value for "company_id" when calling SelectablesApi.get_forms_selectables, must be greater than or equal to 1.'
      end

      allowable_values = ["account_item"]
      ...
      # resource path
      local_var_path = '/api/1/forms/selectables'
      ...
      # return_type
      return_type = opts[:return_type] || 'SelectablesIndexResponse' 
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SelectablesApi#get_forms_selectables\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
