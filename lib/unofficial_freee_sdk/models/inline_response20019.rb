  ...
  class InlineResponse20019
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::InlineResponse20019` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::InlineResponse20019`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'segment_tags')
        if (value = attributes[:'segment_tags']).is_a?(Array)
          self.segment_tags = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @segment_tags.nil?
        invalid_properties.push('invalid value for "segment_tags", segment_tags cannot be nil.')
      end
    ...
    def valid?
      return false if @segment_tags.nil?
      true
    end
