...
module UnofficialFreeeSdk
  class JournalStatusResponse
    ...
    def self.openapi_types
      {
        :'journals' => :'JournalStatusResponseJournals'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::JournalStatusResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::JournalStatusResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'journals')
        self.journals = attributes[:'journals']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @journals.nil?
        invalid_properties.push('invalid value for "journals", journals cannot be nil.')
      end
