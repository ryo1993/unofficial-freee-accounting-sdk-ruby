...
module UnofficialFreeeSdk
  class SegmentTagResponseSegmentTag
    # セグメントタグID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::SegmentTagResponseSegmentTag` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::SegmentTagResponseSegmentTag`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if @shortcut1.to_s.length > 20
      return false if @shortcut2.to_s.length > 20
      true
    end
    ...
    def hash
      [id, name, description, shortcut1, shortcut2].hash
    end
