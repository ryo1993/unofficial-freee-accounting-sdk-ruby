...
module UnofficialFreeeSdk
  class WalletTxnResponse
    ...
    def self.openapi_types
      {
        :'wallet_txn' => :'WalletTxnResponseWalletTxn'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::WalletTxnResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::WalletTxnResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'wallet_txn')
        self.wallet_txn = attributes[:'wallet_txn']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @wallet_txn.nil?
        invalid_properties.push('invalid value for "wallet_txn", wallet_txn cannot be nil.')
      end
    ...
    def valid?
      return false if @wallet_txn.nil?
      true
    end
