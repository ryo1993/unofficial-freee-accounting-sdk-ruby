  ...
  class InlineResponse2004
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::InlineResponse2004` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::InlineResponse2004`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'quotations')
        if (value = attributes[:'quotations']).is_a?(Array)
          self.quotations = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @quotations.nil?
        invalid_properties.push('invalid value for "quotations", quotations cannot be nil.')
      end
    ...
    def valid?
      return false if @quotations.nil?
      true
    end
