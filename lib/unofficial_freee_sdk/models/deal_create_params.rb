  ...
  class DealCreateParams
    ...
    def self.openapi_types
      {
        ...
        :'partner_code' => :'String',
        :'ref_number' => :'String',
        :'details' => :'Array<DealCreateParamsDetails>',
        :'payments' => :'Array<DealCreateParamsPayments>',
        :'receipt_ids' => :'Array<Integer>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::DealCreateParams` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::DealCreateParams`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'receipt_ids')
        if (value = attributes[:'receipt_ids']).is_a?(Array)
    ...
    def hash
      [issue_date, type, company_id, due_date, partner_id, partner_code, ref_number, details, payments, receipt_ids].hash
    end
