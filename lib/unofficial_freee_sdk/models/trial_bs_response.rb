...
module UnofficialFreeeSdk
  class TrialBsResponse
    attr_accessor :trial_bs
    ...
    def self.openapi_types
      {
        :'trial_bs' => :'TrialBsResponseTrialBs'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::TrialBsResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::TrialBsResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'trial_bs')
        self.trial_bs = attributes[:'trial_bs']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @trial_bs.nil?
        invalid_properties.push('invalid value for "trial_bs", trial_bs cannot be nil.')
      end
