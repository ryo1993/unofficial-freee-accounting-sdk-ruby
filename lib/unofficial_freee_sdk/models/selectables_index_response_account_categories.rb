...
module UnofficialFreeeSdk
  class SelectablesIndexResponseAccountCategories
    # 収支
    ...
    def self.openapi_types
      {
        ...
        :'title' => :'String',
        :'desc' => :'String',
        :'account_items' => :'Array<SelectablesIndexResponseAccountItems>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::SelectablesIndexResponseAccountCategories` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::SelectablesIndexResponseAccountCategories`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'account_items')
        if (value = attributes[:'account_items']).is_a?(Array)
    ...
    def valid?
      ...
      balance_validator = EnumAttributeValidator.new('String', ["expense", "income"])
      return false unless balance_validator.valid?(@balance)
      return false if @org_code.nil?
      org_code_validator = EnumAttributeValidator.new('String', ["personal", "corporate"])
      return false unless org_code_validator.valid?(@org_code)
    ...
    def balance=(balance)
      validator = EnumAttributeValidator.new('String', ["expense", "income"])
      unless validator.valid?(balance)
        fail ArgumentError, "invalid value for \"balance\", must be one of #{validator.allowable_values}."
      end
      @balance = balance
    end

    # Custom attribute writer method checking allowed values (enum).
    # @param [Object] org_code Object to be assigned
    def org_code=(org_code)
      validator = EnumAttributeValidator.new('String', ["personal", "corporate"])
      unless validator.valid?(org_code)
        fail ArgumentError, "invalid value for \"org_code\", must be one of #{validator.allowable_values}."
      end
      @org_code = org_code
    end
    ...
    def hash
      [balance, org_code, role, title, desc, account_items].hash
    end
