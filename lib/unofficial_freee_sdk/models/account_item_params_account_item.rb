...
module UnofficialFreeeSdk
  class AccountItemParamsAccountItem
    # 勘定科目名 (30文字以内)
    attr_accessor :name
    ...
    def self.openapi_types
      {
        ...
        :'searchable' => :'Integer',
        :'items' => :'Array<AccountItemParamsAccountItemItems>',
        :'partners' => :'Array<AccountItemParamsAccountItemItems>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::AccountItemParamsAccountItem` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::AccountItemParamsAccountItem`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @tax_name.nil?
        invalid_properties.push('invalid value for "tax_name", tax_name cannot be nil.')
      end
      ...
      if @corresponding_income_name.nil?
        invalid_properties.push('invalid value for "corresponding_income_name", corresponding_income_name cannot be nil.')
      end

      if @corresponding_expense_name.nil?
        invalid_properties.push('invalid value for "corresponding_expense_name", corresponding_expense_name cannot be nil.')
      end

      if !@searchable.nil? && @searchable > 3
        invalid_properties.push('invalid value for "searchable", must be smaller than or equal to 3.')
      end

      if !@searchable.nil? && @searchable < 2
    ...
    def valid?
      ...
      return false if @tax_name.nil?
      return false if @group_name.nil?
      return false if @account_category.nil?
      return false if @corresponding_income_name.nil?
      return false if @corresponding_expense_name.nil?
      return false if !@searchable.nil? && @searchable > 3
      return false if !@searchable.nil? && @searchable < 2
      true
    end
    ...
    def hash
      [name, shortcut, shortcut_num, tax_name, group_name, account_category, corresponding_income_name, corresponding_expense_name, accumulated_dep_account_item_name, searchable, items, partners].hash
    end
