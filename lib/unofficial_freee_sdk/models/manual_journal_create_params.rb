...
module UnofficialFreeeSdk
  class ManualJournalCreateParams
    # 事業所ID
    ...
    def self.openapi_types
      {
        ...
        :'issue_date' => :'String',
        :'adjustment' => :'Boolean',
        :'details' => :'Array<ManualJournalCreateParamsDetails>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::ManualJournalCreateParams` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::ManualJournalCreateParams`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
