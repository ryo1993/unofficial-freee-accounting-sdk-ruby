...
module UnofficialFreeeSdk
  class TrialPlResponse
    attr_accessor :trial_pl
    ...
    def self.openapi_types
      {
        :'trial_pl' => :'TrialPlResponseTrialPl'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::TrialPlResponse` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::TrialPlResponse`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'trial_pl')
        self.trial_pl = attributes[:'trial_pl']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @trial_pl.nil?
        invalid_properties.push('invalid value for "trial_pl", trial_pl cannot be nil.')
      end
