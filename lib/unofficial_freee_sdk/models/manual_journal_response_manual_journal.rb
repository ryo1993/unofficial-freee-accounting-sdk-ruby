...
module UnofficialFreeeSdk
  class ManualJournalResponseManualJournal
    # 振替伝票ID
    attr_accessor :id
    ...
    def self.openapi_types
      {
        ...
        :'adjustment' => :'Boolean',
        :'txn_number' => :'String',
        :'details' => :'Array<ManualJournalResponseManualJournalDetails>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::ManualJournalResponseManualJournal` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::ManualJournalResponseManualJournal`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'txn_number')
        self.txn_number = attributes[:'txn_number']
      end
    ...
    def hash
      [id, company_id, issue_date, adjustment, txn_number, details].hash
    end
