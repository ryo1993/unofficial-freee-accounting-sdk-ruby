...
module UnofficialFreeeSdk
  class PartnerCreateParamsPartnerDocSettingAttributes
    # 請求書送付方法(mail:メール、posting:郵送、mail_and_posting:メールと郵送)
    attr_accessor :sending_method
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::PartnerCreateParamsPartnerDocSettingAttributes` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::PartnerCreateParamsPartnerDocSettingAttributes`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'sending_method')
        self.sending_method = attributes[:'sending_method']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          sending_method == o.sending_method
    end
