...
module UnofficialFreeeSdk
  class TrialPlTwoYearsResponseTrialPlTwoYearsBalances
    # 勘定科目ID(勘定科目の時のみ含まれる)
    ...
    def self.openapi_types
      {
        ...
        :'partners' => :'Array<TrialBsTwoYearsResponseTrialBsTwoYearsPartners>',
        :'items' => :'Array<TrialBsTwoYearsResponseTrialBsTwoYearsItems>',
        :'sections' => :'Array<TrialPlTwoYearsResponseTrialPlTwoYearsSections>',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::TrialPlTwoYearsResponseTrialPlTwoYearsBalances` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::TrialPlTwoYearsResponseTrialPlTwoYearsBalances`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_item_id, account_item_name, partners, items, sections, account_category_id, account_category_name, total_line, hierarchy_level, parent_account_category_id, parent_account_category_name, last_year_closing_balance, closing_balance, year_on_year].hash
    end
