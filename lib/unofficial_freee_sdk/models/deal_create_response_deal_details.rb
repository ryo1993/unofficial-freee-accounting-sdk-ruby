...
module UnofficialFreeeSdk
  class DealCreateResponseDealDetails
    # 取引行ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::DealCreateResponseDealDetails` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::DealCreateResponseDealDetails`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@tax_id.nil? && @tax_id < 1
        invalid_properties.push('invalid value for "tax_id", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@tax_id.nil? && @tax_id < 1
    ...
    def tax_id=(tax_id)
      ...
      if !tax_id.nil? && tax_id < 1
        fail ArgumentError, 'invalid value for "tax_id", must be greater than or equal to 1.'
      end
    ...
    def hash
      [id, account_item_id, tax_id, tax_code, item_id, section_id, tag_ids, segment_1_tag_id, segment_2_tag_id, segment_3_tag_id, amount, vat, description, entry_side].hash
    end
