...
module UnofficialFreeeSdk
  class JournalStatusResponseJournals
    # 受け付けID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::JournalStatusResponseJournals` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::JournalStatusResponseJournals`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @visible_tags.nil?
        invalid_properties.push('invalid value for "visible_tags", visible_tags cannot be nil.')
      end
    ...
    def valid?
      ...
      download_type_validator = EnumAttributeValidator.new('String', ["csv", "generic", "pdf"])
      return false unless download_type_validator.valid?(@download_type)
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["enqueued", "working", "uploaded", "failed"])
    ...
    # @param [Object] download_type Object to be assigned
    def download_type=(download_type)
      validator = EnumAttributeValidator.new('String', ["csv", "generic", "pdf"])
      unless validator.valid?(download_type)
        fail ArgumentError, "invalid value for \"download_type\", must be one of #{validator.allowable_values}."
      end
      @download_type = download_type
    end
    ...
    def status=(status)
      validator = EnumAttributeValidator.new('String', ["enqueued", "working", "uploaded", "failed"])
    ...
    def hash
      [id, company_id, download_type, status, start_date, end_date, visible_tags, download_url].hash
    end
