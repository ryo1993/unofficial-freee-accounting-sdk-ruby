...
module UnofficialFreeeSdk
  class InvoiceResponseInvoiceInvoiceContents
    # 請求内容ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::InvoiceResponseInvoiceInvoiceContents` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::InvoiceResponseInvoiceInvoiceContents`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'reduced_vat')
        self.reduced_vat = attributes[:'reduced_vat']
      end
    ...
    def list_invalid_properties
      ...
      if @reduced_vat.nil?
        invalid_properties.push('invalid value for "reduced_vat", reduced_vat cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @reduced_vat.nil?
    ...
    def hash
      [id, order, type, qty, unit, unit_price, amount, vat, reduced_vat, description, account_item_id, account_item_name, tax_code, item_id, item_name, section_id, section_name, tag_ids, tag_names, segment_1_tag_id, segment_1_tag_name, segment_2_tag_id, segment_2_tag_name, segment_3_tag_id, segment_3_tag_name].hash
    end
