...
module UnofficialFreeeSdk
  class DealCreateResponseDealPayments
    # 取引行ID
    attr_accessor :id
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::DealCreateResponseDealPayments` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::DealCreateResponseDealPayments`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      return false if !@from_walletable_id.nil? && @from_walletable_id > 999999999999
      return false if !@from_walletable_id.nil? && @from_walletable_id < 1
    ...
    def hash
      [id, date, from_walletable_type, from_walletable_id, amount].hash
    end
