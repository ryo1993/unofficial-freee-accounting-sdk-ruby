  ...
  class DealResponseDeal
    ...
    def self.openapi_types
      {
        ...
        :'ref_number' => :'String',
        :'status' => :'String',
        :'details' => :'Array<DealCreateResponseDealDetails>',
        :'renews' => :'Array<DealResponseDealRenews>',
        :'payments' => :'Array<DealCreateResponseDealPayments>',
        :'receipts' => :'Array<DealResponseDealReceipts>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `UnofficialFreeeSdk::DealResponseDeal` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `UnofficialFreeeSdk::DealResponseDeal`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'renews')
        if (value = attributes[:'renews']).is_a?(Array)
          self.renews = value
        end
      end
    ...
    def valid?
      ...
      return false if @amount < -999999999999
      type_validator = EnumAttributeValidator.new('String', ["income", "expense"])
      return false unless type_validator.valid?(@type)
      return false if @partner_id.nil?
      return false if @partner_id > 999999999999
      return false if @partner_id < 1
      return false if @status.nil?
      status_validator = EnumAttributeValidator.new('String', ["unsettled", "settled"])
    ...
    def hash
      [id, company_id, issue_date, due_date, amount, due_amount, type, partner_id, partner_code, ref_number, status, details, renews, payments, receipts].hash
    end
